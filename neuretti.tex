...
In our third group of tests, we calibrated a topology corresponding to...  starfish? \cite{}
...
We show in Figure \ref{fig:topology-starfish} the topology of this neural network, denoting by solid and dashed lines the excitatory and inhibitory connections between the neurons, respectively.
In this test, \name{} failed to identify any optimal solutions, as shown in Figure \ref{fig:target-stellona}. 
As a matter of fact, the spiking sequence differs as the solution found by \name{} is characterized by a slightly different activity (e.g., neuron 4 spikes two times in the simulation during the interval 0--50ms, while only a single spike was present in the target).
